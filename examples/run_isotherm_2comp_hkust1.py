...
from aiida_lsmo_workflows.isotherm_multi_comp_serial import MultiCompIsothermWorkChain
...
zeopp_codename = sys.argv[1]
raspa_codename = sys.argv[2]

try:
    zeopp_code = Code.get_from_string(zeopp_codename)
except NotExistent:
    print("The code '{}' does not exist".format(zeopp_codename))
    sys.exit(1)
...
raspa_comp = {
    "comp1":{"name":"xenon","mol_fraction":0.20,"radius":1.98, "mol_def":"TraPPE", "conv_threshold" : 0.10, "singlebead":True},
    "comp2":{"name":"krypton","mol_fraction":0.80,"radius":1.82, "mol_def":"TraPPE", "conv_threshold" : 0.10, "singlebead":True}
    }
