...
from aiida.work.workchain import WorkChain, ToContext, Outputs, if_
...
class Cp2kDirectCellOptDdecWorkChain(WorkChain):
    """Workchain that for a given matherial will optimize the structure
    and compute ddec charges."""
    @classmethod
    def define(cls, spec):
        super(Cp2kDirectCellOptDdecWorkChain, cls).define(spec)
        ...
        # ddec
        spec.input('ddec_compute', valid_type=Bool, default=Bool(True), required=False)
        spec.input('ddec_code', valid_type=Code)
        spec.input("ddec_parameters", valid_type=ParameterData, default=None, required=False)
        ...
        spec.outline(
            cls.run_cellopt,
            cls.parse_cellopt,
            if_(cls.should_compute_charges)(
    ...
    def run_cellopt(self):
       ...
       # Expand the unit cell so that: min(perpendicular_width) > threshold
        ...
        self.report("pk: {} | Running Cp2kDirectCellOptWorkChainn to optimize cell and geometry".format(running.pid))
    ...
    def should_compute_charges(self):
        """Decide whether to run DDEC calculaton."""
        if self.inputs.ddec_compute == True:
            return True
        else:
            return False
    ...
    def run_point_charges(self):
        ...
        inputs = {
            ...
            'cp2k_parent_folder' : self.ctx.cellopt_calc['remote_folder'],
            # ddec parameters
    ...
    def return_results(self):
        ...
        self.report("Cp2kDirectCellOptDdecWorkChain is completed.")
